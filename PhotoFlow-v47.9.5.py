...
"""
PANOFLOW (v47.9.5)
- NAAM: Gewijzigd naar PanoFlow.
- FIX: Portret-oriëntatie fix voor zowel TIFF als DNG.
- FIX: DNG opslaglocatie direct in reeks-map.
- GUI: Info-sectie geactualiseerd.
"""
...
REQUIRED_TOOLS = ['exiftool', 'darktable-cli', 'align_image_stack', 'enfuse', 'hdrmerge']
...
def reset_and_copy_metadata(src_raw, dst_hdr):
    """Voor TIFF: Kopieert EXIF maar forceert oriëntatie naar 1 (Normal)."""
    try:
        subprocess.run([
...
def copy_metadata_full(src_raw, dst_hdr):
    """Voor DNG: Kopieert alle EXIF inclusief originele oriëntatie."""
    try:
        subprocess.run([
            'exiftool', '-overwrite_original', '-tagsFromFile', src_raw,
            '-all:all', dst_hdr
...
class SortWorker(BaseWorker):
    ...
    def run(self):
        try:
            ...
            self.log.emit(f"PanoFlow: Analysing {self.source_dir}...")
            ...
            for line in filter(None, result.stdout.strip().split('\n')):
                ...
                p = line.split('\t')
                if len(p) < 5 or os.path.splitext(p[0])[1].upper() not in SUPPORTED_EXTS: continue
                photo_list.append({'name': p[0], 'ts': int(datetime.strptime(p[1], "%Y:%m:%d %H:%M:%S").timestamp()), 'exp': f"S{p[2]}A{p[3]}"})
...
class HdrWorker(BaseWorker):
    def __init__(self, base_dir, method, bit_depth, collect, cleanup):
        super().__init__(); self.base_dir, self.method, self.bit_depth, self.collect, self.cleanup = os.path.abspath(base_dir), method, bit_depth, collect, cleanup
    @Slot()
    def run(self):
        try:
            ...
            for i, path in enumerate(subdirs):
                ...
                os.makedirs(cfg)
                if self.method == 'enfuse': self._do_enfuse(path, name, cfg, xmp)
                else: self._do_hdrmerge(path, name)
                self.progress.emit(int(((i + 1) / len(subdirs)) * 80))
            if self._is_running and self.collect: self._do_collect()
        ...
        try:
            ...
            if len(tifs) >= 2 and self._is_running:
                ...
                if res.returncode == 0 and self._is_running:
                    ...
                    subprocess.run(['enfuse', '--exposure-weight=1', '--saturation-weight=0.2', '--hard-mask', '--output', out_h] + sorted(glob.glob(f"{ali}*.tif")), env=ENV_STABLE, capture_output=True)
    ...
    def _do_collect(self):
        self.log.emit("\n--- Collecting Results ---")
        dest = os.path.join(os.path.dirname(self.base_dir), CONFIG["HDR_COLLECT_NAME"]); os.makedirs(dest, exist_ok=True)
        files = glob.glob(os.path.join(self.base_dir, '**', '*_HDR_*.tif'), recursive=True) + glob.glob(os.path.join(self.base_dir, '**', '*_HDR.dng'), recursive=True)
        for f in list(set(files)): shutil.move(f, os.path.join(dest, os.path.basename(f)))
        self.log.emit(f"✓ Results moved to '{CONFIG['HDR_COLLECT_NAME']}'.")

    def _do_cleanup(self):
        marker = os.path.join(self.base_dir, CONFIG["SAFE_MARKER"])
        if os.path.exists(marker):
            self.log.emit("--- Cleaning up ---")
            shutil.rmtree(self.base_dir); self.log.emit("✓ Work folder cleaned.")

# --- GUI ---
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("PanoFlow v47.9.5"); self.setGeometry(100, 100, 800, 600); self.worker = None; self.thread = None
    ...
    def setup_t2(self):
        ...
        self.enf = QWidget(); el = QVBoxLayout(self.enf); el.setContentsMargins(0,0,0,0); el.addWidget(QLabel("Bit Diepte:")); self.bd = QComboBox(); self.bd.addItems(["16", "8"]); el.addWidget(self.bd); l.addWidget(self.enf)
        self.m2.currentIndexChanged.connect(lambda i: self.enf.setVisible(i==0))
    ...
    def info(self):
        txt = (
            "<h3>PanoFlow (v47.9.5)</h3>"
            "<p>Automates management and processing of RAW image files for panorama construction.</p>"
            "<b>1. Sorting:</b> Groups RAW files into sequences. Only the first capture stays in source, creating a clean overview of 'master shots' for standard stitching.<br><br>"
            "<b>2. HDR & Collection:</b> Processes sequences into TIFF or DNG and preserves Portrait orientation. Results are moved to a central folder.<br><br>"
            "<b>XMP:</b> Required for Enfuse to apply lens corrections."
        )
    ...
    def go1(self):
        ...
        self.worker = SortWorker(self.s1.text(), int(self.sc.currentText()), self.k.isChecked())
        self._run(self.p1, self.log1, self.b1, "Start Sorteren"); self.s2.setText(os.path.join(self.s1.text(), CONFIG["SORTED_DIR_NAME"]))

    def go2(self):
        if self.worker: self.worker.stop(); return
        self.b2.setText("Stop Verwerking"); self.log2.clear(); self.thread = QThread()
        self.worker = HdrWorker(self.s2.text(), 'enfuse' if self.m2.currentIndex()==0 else 'hdrmerge', self.bd.currentText(), self.c1.isChecked(), self.c2.isChecked())
        self._run(self.p2, self.log2, self.b2, "Start HDR Verwerking")

    def _run(self, bar, log, btn, txt):
        self.worker.moveToThread(self.thread); self.thread.started.connect(self.worker.run)
        self.worker.finished.connect(lambda: self._end(btn, txt))
        self.worker.log.connect(log.append); self.worker.progress.connect(bar.setValue); self.thread.start()

    def _end(self, b, t):
        self.thread.quit(); self.thread.wait(); self.worker = None; b.setText(t); b.setEnabled(True)
