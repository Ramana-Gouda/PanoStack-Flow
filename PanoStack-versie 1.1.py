...
PanoStack (v1.1)
- NEW: Versie 1.1.
- FIX: Hugin GUI omzeilt RAW-keuzemenu door automatische DT-voorverwerking.
- FIX: Photometrische optimalisatie (-p) voor naadloze overgangen.
- FIX: Darktable knop hersteld en UI uitgelijnd.
"""
...
class SortWorker(BaseWorker):
    ...
    def run(self):
        try:
            self.log.emit(f"<b style='color:#2980b9;'>Sorteren gestart:</b> {self.source_dir}")
            ...
            if not all_files: self.log.emit("<i style='color:#e67e22;'>Geen ondersteunde bestanden gevonden.</i>"); self.finished.emit(); return
            photos = []
            ...
            self.log.emit("<br><b style='color:#27ae60;'>✓ Sorteren voltooid.</b>")
        except Exception as e: self.log.emit(f"<b style='color:#e74c3c;'>Fout bij sorteren: {e}</b>")
    ...
    def _process_group(self, group, dest_root, seq):
        ...
        self.log.emit(f"  -> <span style='color:#34495e;'>{type_p}</span> ({len(group)} foto's)")
        for f in group:
...
class HdrBurstWorker(BaseWorker):
    ...
    def run(self):
        try:
            ...
            if not subdirs: self.log.emit(f"<b style='color:#e67e22;'>Geen mappen gevonden die starten met {prefix}.</b>"); self.finished.emit(); return
            self.log.emit(f"<b style='color:#2980b9;'>Verwerken:</b> {len(subdirs)} mappen.")
            ...
            for i, path in enumerate(subdirs):
                if not self._is_running: break
                name = os.path.basename(path); self.log.emit(f"<b>Actief:</b> {name}"); xmp = find_best_xmp(path)
        ...
        except Exception as e: self.log.emit(f"<b style='color:#e74c3c;'>Fout: {e}</b>")
        finally: self.finished.emit()
...
class PanoWorker(BaseWorker):
    ...
    def run(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            imgs = []
            try:
                self.log.emit(f"<b style='color:#2980b9;'>Stitchen gestart (8-bit):</b> {len(self.files)} foto's.")
                ...
                if len(imgs) > 1 and self._is_running:
                    ...
                    if status == cv2.Stitcher_OK:
                        ...
                        cv2.imwrite(out, res)
                        self.log.emit(f"<b style='color:#27ae60;'>✓ Opgeslagen in: {CONFIG['PANO_COLLECT_NAME']}</b>")
                        self.result_path.emit(out)
                    else: self.log.emit("<b style='color:#e74c3c;'>Fout: Stitchen mislukt.</b>")
    ...
    def run(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            try:
                ...
                pto = os.path.join(tmp_dir, "project.pto"); prefix = os.path.join(tmp_dir, "output")
                self.log.emit("<i> -> Project genereren...</i>"); self.safe_run(['pto_gen', '-o', pto] + tiffs); self.sub_progress.emit(30)
                self.log.emit("<i> -> Controlepunten zoeken...</i>"); self.safe_run(['cpfind', '--multirow', '-o', pto, pto]); self.sub_progress.emit(50)
                self.log.emit("<i> -> Photometrische Optimalisatie...</i>")
                self.safe_run(['autooptimiser', '-a', '-m', '-p', '-s', '-o', pto, pto])
                self.safe_run(['pano_modify', '--canvas=AUTO', '--crop=AUTO', '--center', '-o', pto, pto]); self.sub_progress.emit(70)
                self.log.emit("<i> -> Stitchen final 16-bit TIFF...</i>")
                ...
                if os.path.exists(res_tif):
                    ...
                    self.log.emit(f"<b style='color:#27ae60;'>✓ Hugin CLI voltooid.</b>")
                    self.result_path.emit(out)
                else: self.log.emit("<b style='color:#e74c3c;'>Fout: Hugin CLI mislukt.</b>")
            except Exception as e: self.log.emit(f"<b style='color:#e74c3c;'>Fout: {e}</b>")
            finally: self.progress.emit(100); self.finished.emit()

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("PanoStack v1.1"); self.resize(1300, 900)
    ...
    def setup_t1(self):
        ...
        h = QHBoxLayout(); h.addWidget(QLabel("Bron:")); h.addWidget(self.s1); b = QPushButton("..."); b.clicked.connect(lambda: self.sel(self.s1)); h.addWidget(b)
    ...
    def setup_t2(self):
        ...
        h_main.addWidget(QLabel("Map:")); self.s2 = QLineEdit(); h_main.addWidget(self.s2); b = QPushButton("..."); b.clicked.connect(lambda: self.sel(self.s2)); h_main.addWidget(b)
        ...
        v_prog = QWidget(); v_prog.setFixedHeight(60); vp_l = QVBoxLayout(v_prog); vp_l.setContentsMargins(0,0,0,0); vp_l.setSpacing(1); vp_l.addWidget(QLabel("Totaal:")); self.p2 = self._make_thin_bar(); vp_l.addWidget(self.p2); vp_l.addWidget(QLabel("Huidig:"))
    ...
    def setup_t3(self):
        ...
        h.addWidget(QLabel("Map:")); self.s3 = QLineEdit(); h.addWidget(self.s3); b = QPushButton("..."); b.clicked.connect(lambda: self.sel(self.s3)); h.addWidget(b)
        ...
        v_prog3 = QWidget(); v_prog3.setFixedHeight(60); vp3_l = QVBoxLayout(v_prog3); vp3_l.setContentsMargins(0,0,0,0); vp3_l.setSpacing(1); vp3_l.addWidget(QLabel("Totaal:")); self.p3 = self._make_thin_bar(); vp3_l.addWidget(self.p3); vp3_l.addWidget(QLabel("Huidig:"))
    ...
    def setup_t4(self):
        ...
        h_paths.addWidget(QLabel("Verzamelmap:")); self.s4 = QLineEdit(); h_paths.addWidget(self.s4); b1 = QPushButton("..."); b1.clicked.connect(lambda: self.sel(self.s4)); h_paths.addWidget(b1)
        self.lbl_x4 = QLabel("Vrije XMP:"); h_paths.addWidget(self.lbl_x4); self.x4 = QLineEdit(); h_paths.addWidget(self.x4);
        self.b_xmp4 = QPushButton("Kies"); self.b_xmp4.clicked.connect(self.sel_xmp4); h_paths.addWidget(self.b_xmp4); main_l.addLayout(h_paths)
        ...
        h_opts.addWidget(self.f4); h_opts.addWidget(QLabel("Grootte:")); self.ts4 = QComboBox(); self.ts4.addItems(["100", "200", "300"]); self.ts4.setCurrentText("200"); self.ts4.currentIndexChanged.connect(self.refresh_t4); h_opts.addWidget(self.ts4); self.p4_load = self._make_thin_bar(); h_opts.addWidget(self.p4_load); main_l.addLayout(h_opts)
        ...
        self.b_dt = QPushButton("Darktable", clicked=self.open_in_darktable); self.b_dt.setFixedWidth(90); btn_h.addWidget(self.b_dt)
        self.b_hu = QPushButton("Start Hugin (16 Bit)", clicked=self.open_in_hugin); btn_h.addWidget(self.b_hu)
        self.cb_hugin_gui = QCheckBox("Open GUI"); self.cb_hugin_gui.setFixedWidth(90); btn_h.addWidget(self.cb_hugin_gui)
    ...
    def open_in_hugin(self):
        ...
        if not files:
            QMessageBox.warning(self, "Selectie", "Selecteer eerst foto's."); return

        if self.cb_hugin_gui.isChecked():
            # Voor de GUI: als er RAWs bij zitten, converteren we ze eerst tijdelijk naar TIFF met Darktable.
            # Dit voorkomt dat Hugin vraagt welke RAW-converter (dcraw/DT/RT) gebruikt moet worden.
            raw_files = [f for f in files if f.lower().endswith(('.dng', '.rw2', '.arw', '.cr2', '.cr3', '.nef', '.orf', '.raf'))]
            if raw_files:
                self.log4.append("<i>RAW bestanden gedetecteerd. Voorbereiden voor Hugin GUI via Darktable...</i>")
                tmp_hugin_dir = tempfile.mkdtemp(prefix="panostack_hugin_")
                final_files = []
                for i, f in enumerate(files):
                    if f in raw_files:
                        t = os.path.join(tmp_hugin_dir, f"hugin_ready_{i}.tif")
                        xmp = self.x4.text() if (self.x4.text() and os.path.exists(self.x4.text())) else find_best_xmp(os.path.dirname(f))
                        cmd = ['darktable-cli', f, t, '--core', '--library', ':memory:', '--disable-opencl']
                        if xmp: cmd.insert(2, xmp)
                        subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                        final_files.append(t)
                    else:
                        final_files.append(f)
                subprocess.Popen(['hugin'] + final_files)
            else:
                subprocess.Popen(['hugin'] + files)
        else:
            self.log4.clear()
            pano_dir = self._get_pano_save_dir(files[0])
            w = HuginCliWorker(files, output_dir=pano_dir, custom_xmp=self.x4.text())
    ...
    def show_info(self):
        info = f"""
        <h2 style='color:#2980b9;'>PanoStack v1.1</h2>
        <p>RAW workflow tool voor Arch Linux.</p>
        <ul>
            <li><b>Hugin GUI:</b> Gebruikt nu altijd Darktable voor RAW conversie (dcraw omzeild).</li>
            <li><b>Hugin CLI:</b> Inclusief photometrische optimalisatie (-p) voor egale overlappen.</li>
            <li><b>Darktable knop:</b> Directe nabewerking van het panorama.</li>
        </ul>
        """
    ...
    def clear_prev_label(self, w):
        ...
        target.clear(); target.setText("Verwerken..."); target.setStyleSheet("color: #7f8c8d; font-style: italic; font-size: 14px;")
