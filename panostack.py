...
- NEW: EV-correction options: +1 EV, -1 EV, -2 EV, -3 EV via right-click (DNG only).
- FIX: EV correction + Free XMP integration via history injection.
- FIX: Darktable-CLI command order: <input> <xmp> <output> (Optimal for v5.6.0).
...
- FIX: Free XMP field alway enabled in Panorama tab.
...
- NEW: Double-click to mark DNG/TIFF for deletion in Pano-tab (RAW protected).
- NEW: Exclude RAW sequence from TIFF-HDR processing via Right-click (v3.6.0).
- FIX: RAW (enkel) now correctly shows only 1st frame of stacks and unique singles.
- FIX: RAW (Serie) now correctly filtered to show only files in Serie_ folders.
- FIX: Start Darktable button now correctly links XMP to RAW files for GUI preview.
"""
...
class SortWorker(BaseWorker):
    ...
    def run(self):
        try:
            ...
            self.log.emit(f"Metadata lezen van {len(all_paths)} bestanden...")
            photos = []
            ...
            self.log.emit("Chronologisch groeperen...")
    ...
    def _process_group(self, group, dest_root, seq):
        ...
        self.log.emit(f"  -> {type_p}_{seq:03d}: {len(group)} foto's verwerkt.")
        for f in group:
...
class HdrBurstWorker(BaseWorker):
    ...
    def run(self):
        try:
            ...
            search_prefixes = (prefix, "_" + prefix)
            subdirs = []
            if any(os.path.basename(self.base_dir).startswith(p) for p in search_prefixes): subdirs.append(self.base_dir)
            for r, ds, _ in os.walk(self.base_dir):
                for d in ds:
                    if any(d.startswith(p) for p in search_prefixes):
                        subdirs.append(os.path.join(r, d))
            subdirs = sorted(list(set(subdirs)))
            if not subdirs:
                self.log.emit(f"<b style='color:#e67e22;'>Geen {prefix} mappen gevonden in {self.base_dir}.</b>")
            ...
            self.log.emit(f"<b style='color:#2980b9;'>{self.mode} Stacking gestart:</b> {len(subdirs)} mappen gevonden.")
            for i, path in enumerate(subdirs):
                if not self._is_running: break

                files_in_dir = sorted([f for f in os.listdir(path) if any(f.lower().endswith(ex) for ex in VALID_EXTS) and "_HDR" not in f and "_Burst" not in f])
                if not files_in_dir: continue
                first_file_basename = os.path.splitext(files_in_dir[0])[0]

                if self.mode == "HDR":
                    out_name = f"{first_file_basename}_HDR"
                else:
                    out_name = f"{first_file_basename}_Burst"

                xmp = find_best_xmp()
                self.log.emit(f"<br><b>Actief:</b> {os.path.basename(path)} ({out_name})")

                coll_dng = os.path.join(coll_root, "DNG", f"{out_name}.dng")
                coll_tif = os.path.join(coll_root, "TIFF", f"{out_name}.tif")
                ...
                if self.mode == "HDR" and ("hdrmerge" in self.method or "beide" in self.method) and not os.path.exists(coll_dng):
                    ...
                    self.log.emit(f"  -> HDRmerge (DNG) met {len(raws)} bestanden...")
                    ...
                    if self.safe_run(['hdrmerge', '-b', '16', '-o', out] + raws) == 0:
                        ...
                        self.log.emit(f"  -> ✓ DNG opgeslagen in Verzamelmap.")
                if (self.mode == "BURST" or "enfuse" in self.method or "beide" in self.method):
                    if os.path.basename(path).startswith("_"):
                        self.log.emit(f"  -> <i style='color:#e67e22;'>TIFF overgeslagen (Map gemarkeerd voor uitsluiting)</i>")
                    elif not os.path.exists(coll_tif):
                        self.log.emit(f"  -> Enfuse/Median (TIFF) workflow gestart...")
                        res = self._do_enfuse(path, out_name, xmp)
                        if res:
                            smart_copy(res, coll_tif); self.result_path.emit(res)
                            self.log.emit(f"  -> ✓ TIFF opgeslagen in Verzamelmap.")
                self.progress.emit(int(((i + 1) / len(subdirs)) * 100))
            self.log.emit("<br><b style='color:#27ae60;'>✓ HDR/Burst voltooid.</b>")
        except Exception as e: self.log.emit(f"Fout: {e}")
        finally: self.finished.emit()

    def _do_enfuse(self, path, out_name, xmp):
        all_f = sorted([f for f in os.listdir(path) if any(f.lower().endswith(ex) for ex in VALID_EXTS) and "_HDR" not in f and "_Burst" not in f])
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for idx, f in enumerate(files):
                ...
                self.log.emit(f"     Ontwikkelen ({idx+1}/{len(files)}): {f}")
    ...
    def run(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                self.log.emit(f"Bestanden in selectie: {len(self.files)}")
                for i, f in enumerate(self.files):
                    if not self._is_running: break
                    ext_low = f.lower()
                    is_dng = ext_low.endswith('.dng')
                    is_raw = any(ext_low.endswith(ex) for ex in ('.rw2', '.arw', '.cr2', '.cr3', '.nef', '.orf', '.raf'))
                    ev = self.ev_map.get(f, 0) if is_dng else 0
    ...
    def run(self):
        with tempfile.TemporaryDirectory() as tmp:
            try:
                ...
                self.log.emit(f"Selectie bevat {len(self.files)} bestanden.")
                tiffs = []
                for i, f in enumerate(self.files):
                    if not self._is_running: break
                    ext_low = f.lower()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PanoStack v3.6.0")
    ...
    def setup_t3(self):
        ...
        self.bl3 = QComboBox(); self.bl3.addItems(["8", "16"]); h.addWidget(QLabel("Limiet:")); h.addWidget(self.bl3); l.addLayout(h)
    ...
    def setup_t4(self):
        ...
        self.v_split = QSplitter(Qt.Vertical); self.lw = QListWidget(); self.lw.setViewMode(QListWidget.IconMode); self.lw.setIconSize(QSize(200, 200)); self.lw.setSelectionMode(QAbstractItemView.MultiSelection); self.lw.setContextMenuPolicy(Qt.CustomContextMenu); self.lw.customContextMenuRequested.connect(self.show_context); self.lw.itemDoubleClicked.connect(self.mark_for_deletion)
        ...
        self.b_dt = QPushButton("Start Darktable"); self.b_dt.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed); self.b_dt.clicked.connect(self.open_dt); btn_h.addWidget(self.b_dt)
        self.b_del = QPushButton("Verwijder Gemarkeerd"); self.b_del.setFixedWidth(140); self.b_del.clicked.connect(self.do_delete_marked); btn_h.addWidget(self.b_del); bot_l.addLayout(btn_h)
    ...
    def show_context(self, pos):
        ...
        if path.lower().endswith('.dng'):
            a_p1 = QAction("+1 Stop (EV)", self); a0 = QAction("Normal (0 EV)", self); a_m1 = QAction("-1 Stop (EV)", self); a_m2 = QAction("-2 Stops (EV)", self); a_m3 = QAction("-3 Stops (EV)", self); menu.addActions([a_p1, a0, a_m1, a_m2, a_m3])
            action = menu.exec(self.lw.mapToGlobal(pos))
        ...
        elif self.f4.currentIndex() == 3: # RAW (enkel)
            parent = os.path.normpath(os.path.dirname(path))
            folder_name = os.path.basename(parent)
            v_prefixes = ("Reeks_", "Burst_", "Serie_")
            e_prefixes = ("_Reeks_", "_Burst_", "_Serie_")
            is_regular = any(folder_name.startswith(p) for p in v_prefixes)
            is_excluded = any(folder_name.startswith(p) for p in e_prefixes)
            if is_regular:
                a_excl = QAction("Sluit reeks uit van TIFF-HDR", self); menu.addAction(a_excl)
                if menu.exec(self.lw.mapToGlobal(pos)) == a_excl:
                    new_parent = os.path.join(os.path.dirname(parent), "_" + folder_name)
                    new_full_path = os.path.join(new_parent, os.path.basename(path))
                    try:
                        os.rename(parent, new_parent)
                        item.setData(Qt.UserRole, new_full_path)
                        item.setBackground(QBrush(QColor(50, 50, 50, 150))); item.setText(f"[UITGESLOTEN] {os.path.basename(path)}")
                    except: pass
            elif is_excluded:
                a_incl = QAction("Reeks weer insluiten voor TIFF-HDR", self); menu.addAction(a_incl)
                if menu.exec(self.lw.mapToGlobal(pos)) == a_incl:
                    new_parent = os.path.join(os.path.dirname(parent), folder_name[1:])
                    new_full_path = os.path.join(new_parent, os.path.basename(path))
                    try:
                        os.rename(parent, new_parent)
                        item.setData(Qt.UserRole, new_full_path)
                        item.setBackground(QBrush(Qt.transparent)); item.setText(os.path.basename(path))
                    except: pass
    def mark_for_deletion(self, item):
        path = item.data(Qt.UserRole)
        ext = os.path.splitext(path)[1].lower()
        if ext not in ['.dng', '.tif', '.tiff']: return
        res = QMessageBox.question(self, "Verwijderen?", f"Wilt u {os.path.basename(path)} markeren voor verwijdering?", QMessageBox.Yes | QMessageBox.No)
        if res == QMessageBox.Yes:
            is_marked = item.data(Qt.UserRole + 2) or False
            if not is_marked:
                item.setData(Qt.UserRole + 2, True); item.setBackground(QBrush(QColor(255, 0, 0, 100)))
                item.setText(f"[VERWIJDEREN] {os.path.basename(path)}")
            else:
                item.setData(Qt.UserRole + 2, False); item.setBackground(QBrush(Qt.transparent))
                ev = item.data(Qt.UserRole + 1) or 0
    ...
    def do_delete_marked(self):
        ...
        for i in range(self.lw.count()):
            it = self.lw.item(i)
            if it.data(Qt.UserRole + 2): marked.append(it)
        if not marked: return
        res = QMessageBox.warning(self, "Bevestig Verwijdering", f"Weet u zeker dat u deze {len(marked)} bestanden definitief wilt verwijderen van schijf?", QMessageBox.Yes | QMessageBox.No)
        if res == QMessageBox.Yes:
            for it in marked:
    ...
    def show_readme(self):
        text = """<h2 style='color:#2980b9;'>PanoStack v3.5.9 - User Manual</h2>
        ...
        <ul>
            ...
            <li><b>Engine:</b> 16-bit Hugin CLI (auto-leveling) or fast Ultra-HQ OpenCV.</li>
        </ul>
    ...
    def update_refresh_all(self):
        sz = int(self.ts4.currentText()); self.lw.setIconSize(QSize(sz, sz))
        self.update_pano_xmp_visibility(); self.refresh_t4()
    ...
    def update_pano_xmp_visibility(self):
        ...
        can_use_xmp = (mode != 0)
        self.x4.setEnabled(can_use_xmp); self.b_xmp4.setEnabled(can_use_xmp); self.lbl_x4.setEnabled(can_use_xmp)
        if not can_use_xmp: self.x4.setStyleSheet("background-color: #2c3e50; color: #95a5a6; border: 1px solid #1a252f;")
        else: self.x4.setStyleSheet("")
        can_delete = (mode in [0, 1])
        self.b_del.setEnabled(can_delete)
        if not can_delete: self.b_del.setStyleSheet("background-color: #2c3e50; color: #7f8c8d; border: 1px solid #1a252f;")
        else: self.b_del.setStyleSheet("")
    ...
    def refresh_t4(self):
        ...
        # Filter aangescherpt voor Serie modus (Mode 2) en Enkel modus (Mode 3)
        if mode == 3: scan_p, rec, inc_first, f_filter = root, False, True, ("Reeks_", "Serie_", "Burst_", "_Reeks_", "_Serie_", "_Burst_")
        elif mode == 2: scan_p, rec, f_filter = os.path.normpath(os.path.join(root, CONFIG["SORTED_DIR_NAME"])), True, ("Serie_", "_Serie_")
    ...
    def add_thumb(self, n, p, img):
        ...
        parent_name = os.path.basename(os.path.normpath(os.path.dirname(p)))
        if parent_name.startswith("_"):
            it.setBackground(QBrush(QColor(50, 50, 50, 150))); it.setText(f"[UITGESLOTEN] {n}")
        self.lw.addItem(it)
    def stop_proc(self):
        if self.worker: self.worker.stop()
    def go1(self):
    ...
    def go_proc(self, mode):
        ...
        limit = int(self.bl3.currentText()) if mode == "BURST" else 0
        w = HdrBurstWorker(p, mode, meth, "16", 1.5, limit, (1.0, 0.2, 0.1))
    ...
    def open_dt(self):
        ...
        try:
            ...
            target = sel[0].data(Qt.UserRole) if len(sel) == 1 else self.last_pano_result
            if target and os.path.exists(target):
                # FIX: Als het een RAW/DNG is, koppel de XMP voor de GUI preview
                ext = os.path.splitext(target)[1].lower()
                is_raw = any(ext == ex for ex in ('.rw2', '.arw', '.cr2', '.cr3', '.nef', '.orf', '.raf', '.dng'))
                if is_raw:
                    xmp_src = self.x4.text() if (self.x4.text() and os.path.exists(self.x4.text())) else find_best_xmp()
                    if xmp_src:
                        xmp_dst = target + ".xmp"
                        # Alleen kopieren als er nog geen sidecar is (of overschrijven voor actuele 'oppepper')
                        try: shutil.copy2(xmp_src, xmp_dst)
                        except: pass

                subprocess.Popen(['darktable', '--library', ':memory:', target], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except: pass
    def go_hugin(self):
        items = self.lw.selectedItems()
        if not items: return
...
class ThumbnailWorker(QObject):
    ...
    def run(self):
        ...
        # Stap 1: Als inc_first aan staat (RAW enkel), scan submappen en neem ALLEEN de eerste
        if self.inc_first:
            ...
            if os.path.exists(s_root):
                for r, ds, fs in os.walk(s_root):
                    ...
                    if self.folder_filter and any(bn.startswith(f) for f in self.folder_filter):
                        valid_fs = sorted([f for f in fs if f.lower().endswith(self.extensions)])
                        if valid_fs:
                            f = valid_fs[0] # Alleen de eerste van de stack
                            p = os.path.join(r, f)
                            fps.append(p)
                            found_basenames.add(f)

        # Stap 2: Scan de opgegeven map (hoofdmap voor enkel, of sorted map voor serie)
        if self.recursive and not self.inc_first: # Voor RAW Serie (Mode 2)
            for r, _, fs in os.walk(self.directory):
                if self.is_aborted: break
                bn = os.path.basename(r)
                if self.folder_filter and not any(bn.startswith(f) for f in self.folder_filter): continue
                for f in sorted(fs):
                    if f.lower().endswith(self.extensions):
                        fps.append(os.path.join(r, f))
        elif not self.recursive: # Voor RAW Enkel (singles in hoofdmap)
            for f in sorted(os.listdir(self.directory)):
                if self.is_aborted: break
                if f.lower().endswith(self.extensions):
                    # Alleen toevoegen als het geen kopie is van een stack-foto
                    if f not in found_basenames:
                        fps.append(os.path.join(self.directory, f))
